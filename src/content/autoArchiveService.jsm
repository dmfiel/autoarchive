...
const { MessageArchiver } = ChromeUtils.import('resource:///modules/MessageArchiver.jsm');
...
const autoArchiveService = {
  ...
  preStart: function (time) {
    ...
    const hibernate = autoArchivePref.options.hibernate;
    if (hibernate == -1) return this.updateStatus(this.STATUS_HIBERNATE, 'Schedule Disabled'); // never start schdule
      ...
      const date = new Date(hibernate * 1000);
  ...
  waitTillIdle: function () {
    const idleTime = Math.round(this.idleService.idleTime / 1000);
    const needDelay = autoArchivePref.options.idle_delay - idleTime;
  ...
  removeFolderListener: function (listener) {
    ...
    const index = self.folderListeners.indexOf(listener);
    ...
    try {
      ...
      if (this.serverStatus._listeners_) {
        this.serverStatus._listeners_.forEach(function (listener) {
          const url = listener.URI.QueryInterface(Ci.nsIMsgMailNewsUrl);
  ...
  folderListener: {
    ...
    OnItemEvent: function (folder, event) {
      ...
      if (updateNext) {
        for (const uri in self.wait4Folders) {
          let success = false;
          try {
            const folder = MailUtils.getExistingFolder(uri);
  ...
  copyListener: function (group) { // this listener is for Copy/Delete/Move actions
    ...
    const numberOfMessages = group.messages.length;
    this.msgsDeleted = function (aMsgList) { // Ci.nsIMsgFolderListener, for realDelete message, thus can't get onStopCopy/msgsMoveCopyCompleted
      ...
      for (let iMsgHdr = 0; iMsgHdr < aMsgList.length; iMsgHdr++) {
        const msgHdr = aMsgList.queryElementAt(iMsgHdr, Ci.nsIMsgDBHdr);
        const index = group.messages.indexOf(msgHdr);
  ...
  serverListener: function (key) {
    ...
    this.OnStopRunningUrl = function (uri, aExitCode) {
      ...
      const index = self.serverStatus._listeners_.indexOf(this);
  ...
  checkServers: function () {
    ...
    this.getFoldersFromWait4Folders().some(function (folder) {
      ...
      if (needCheck) {
        ...
        if (server.serverBusy || (server instanceof Ci.nsIPop3IncomingServer && server.runningProtocol)) { self.serverStatus[server.key] = { OK: true, time: Date.now() }; } else servers[server.key] = server;
      }
      return false;
    });
    if (hasBad) {
    ...
    for (const key in servers) {
      try {
        ...
        const server = servers[key]; const listener = new self.serverListener(key); let URI;
        ...
        if (server instanceof Ci.nsIPop3IncomingServer) {
          ...
          const download = (server.loginAtStartUp || server.doBiff) && server.downloadOnBiff && !headers_only;
  ...
  searchListener: function (rule, srcFolder, destFolder) {
    ...
    let searchHit = 0;
    const duplicateHit = [];
    const skipReason = { duplicate: 0, exceed: 0, cantDelete: 0, deleted: 0, srcLocked: 0, destLocked: 0, flaged: 0, unread: 0, tags: 0, cantArchive: 0, offline: 0 };
    let actionSize = 0;
    const listener = this;
    ...
    this.onSearchHit = function (msgHdr, folder) {
      ...
      const age = (Date.now() / 1000 - msgHdr.dateInSeconds) / 3600 / 24;
      ...
      if (['copy', 'move'].indexOf(rule.action) >= 0) {
        ...
        let realDest = rule.dest; let additonal = ''; const additonalNames = [];
        ...
        if (supportHierarchy) {
          ...
          let tmpFolder = msgHdr.folder; const rootFolder = tmpFolder.server.rootFolder;
        ...
        const realDestFolder = MailUtils.getExistingFolder(realDest);
    ...
    this.OnItemAdded = function (parentFolder, childFolder) {
      ...
      for (const uri in this.missingFolders) {
        let folder = destFolder; const names = this.missingFolders[uri];
        const asyncCreating = names.some(function (folderName) {
          try {
    ...
    this.processHeaders = function () {
      try {
        if (rule.action != 'archive') {
          ...
          self.copyGroups = [];
          const groups = {}; // { src => dest : 0, src2 => dest2: 1 }
          const removingDuplicate = (rule.action == 'move' && autoArchivePref.options.delete_duplicate_in_src && duplicateHit.length); const actions = {};
          const messages = this.messages;
          if (removingDuplicate) messages.push.apply(messages, duplicateHit);
          messages.forEach(function (msgHdr) {
            let dest = listener.messagesDest[msgHdr.folder.URI + ' _ ' + msgHdr.messageId] || rule.dest || ''; let action = rule.action;
            if (removingDuplicate && duplicateHit.indexOf(msgHdr) >= 0) {
              dest = '';
              action = 'delete';
            }
            if (dest.length) self.accessedFolders[dest] = true;
            const key = msgHdr.folder.URI + (['copy', 'move'].indexOf(action) >= 0 ? ' => ' + dest : '');
          ...
          self.hookedFunctions.push(autoArchiveaop.around({ target: batchMover, method: 'processNextBatch' }, function (invocation) {
            ...
            try {
              if (!batchMover.awsome_auto_archive_getFolders) {
                for (const key in batchMover._batches) {
                  ...
                  let { srcFolder, archiveFolderURI, granularity, keepFolderStructure, yearFolderName: msgYear, monthFolderName: msgMonth } = batchMover._batches[key];
                  ...
                  const archiveFolder = MailUtils.getExistingFolder(archiveFolderURI, false);
                  ...
                  if (archiveFolder.canCreateSubfolders && keepFolderStructure) {
                    ...
                    const inboxFolder = srcFolder.server.rootMsgFolder.getFolderWithFlags(Ci.nsMsgFolderFlags.Inbox);
                    let folder = srcFolder; const folderNames = [];
          ...
          if (mail3PaneWindow.gFolderDisplay) {
            self.hookedFunctions.push(autoArchiveaop.after({ target: mail3PaneWindow.gFolderDisplay, method: 'hintMassMoveStarting' }, function (result) {
  ...
  doCopyDeleteMoveOne: function (group) {
    ...
    const srcFolder = MailUtils.getExistingFolder(group.src); let destFolder = null;
    ...
    const xpcomHdrArray = toXPCOMArray(group.messages, Ci.nsIMutableArray);
    const mail3PaneWindow = Services.wm.getMostRecentWindow('mail:3pane');
    let msgWindow = null;
    ...
    if (group.action == 'delete') {
      ...
      const deleteModel = srcFolder.server.getIntValue('delete_model');
      ...
      if (!isTrashFolder) {
        const trashFolder = srcFolder.rootFolder.getFolderWithFlags(Ci.nsMsgFolderFlags.Trash);
      ...
      const realDelete = deleteModel == 2 || isTrashFolder;
    ...
    const isMove = (group.action == 'move') && srcFolder.canDeleteMessages;
    try {
  ...
  reportSummaryAndStartNext: function () {
    ...
    for (const action in this.summary) {
    ...
    if (autoArchivePref.options.dry_run || self.dry_run) {
      ...
      const majorVersion = parseInt(browserInfo.version.split('.', 1)[0]);
      const xulSuffix = (majorVersion >= 69 ? 'xhtml' : 'xul');
      if (mail3PaneWindow) mail3PaneWindow.openDialog('chrome://awsomeAutoArchive/content/autoArchiveInfo.' + xulSuffix, '_blank', 'chrome,modal,resizable,centerscreen,dialog=no', this.dryRunLogItems);
    ...
    const delay = this.isExceed ? autoArchivePref.options.start_exceed_delay : autoArchivePref.options.start_next_delay;
    this.clear();
  ...
  _doMoveOrArchiveOne: function () {
    ...
    let srcFolder = null; let destFolder = null;
    try {
      ...
      if (['move', 'copy'].indexOf(rule.action) >= 0) {
        ...
        if (autoArchiveUtil.folderExists(destFolder)) {
          ...
          if (rule.sub == 2) {
            const folders = destFolder.descendants;
            for (const folder of fixIterator(folders || [], Ci.nsIMsgFolder)) {
    ...
    const searchSession = Cc['@mozilla.org/messenger/searchSession;1'].createInstance(Ci.nsIMsgSearchSession);
    if (srcFolder.flags & Ci.nsMsgFolderFlags.Virtual) { // searchSession.addScopeTerm won't work for virtual folder
      const virtFolder = VirtualFolderHelper.wrapVirtualFolder(srcFolder);
      const scope = virtFolder.onlineSearch ? Ci.nsMsgSearchScope.onlineMail : Ci.nsMsgSearchScope.offlineMail;
      ...
      const terms = virtFolder.searchTerms;
      // for (let term of fixIterator(terms, Ci.nsIMsgSearchTerm)) {
      const count = terms.Count();
      if (count) {
        for (let i = 0; i < count; i++) {
          const term = terms.GetElementAt(i).QueryInterface(Ci.nsIMsgSearchTerm);
      ...
      if (rule.sub) {
        const folders = srcFolder.descendants;
        for (const folder of fixIterator(folders || [], Ci.nsIMsgFolder)) {
    ...
    const normal = { subject: Ci.nsMsgSearchAttrib.Subject, from: Ci.nsMsgSearchAttrib.Sender, recipient: Ci.nsMsgSearchAttrib.ToOrCC, size: Ci.nsMsgSearchAttrib.Size, tags: Ci.nsMsgSearchAttrib.Keywords };
    ['subject', 'from', 'recipient', 'size', 'tags'].forEach(function (filter) {
      if (typeof (rule[filter]) !== 'undefined' && rule[filter] != '') {
        ...
        let customId; let positive = true; let attribute = rule[filter];
        if (attribute[0] == '!') {
          positive = false;
        ...
        if (customId) autoArchiveUtil.addSearchTerm(searchSession, { type: Ci.nsMsgSearchAttrib.Custom, customId: customId }, attribute, positive ? Ci.nsMsgSearchOp.Matches : Ci.nsMsgSearchOp.DoesntMatch);
          ...
          if (filter == 'subject') autoArchiveUtil.addSearchTerm(searchSession, normal[filter], attribute, positive ? Ci.nsMsgSearchOp.Contains : Ci.nsMsgSearchOp.DoesntContain);
            ...
            const value = autoArchiveUtil.sizeToKB(attribute);
