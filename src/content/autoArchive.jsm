...
const autoArchive = {
  ...
  realLoad: function (aWindow) {
    try {
      ...
      const status_bar = doc.getElementById('status-bar');
      ...
      if (status_bar) { // add status bar icon
        ...
        const statusbarIcon = doc.createElementNS(XULNS, 'image');
        ...
        aWindow._autoarchive.statusCallback = function (status, detail) {
          ...
          if (autoArchivePref.options.update_statusbartext && aWindow.MsgStatusFeedback && (status != autoArchiveService.STATUS_RUN || !Cc['@mozilla.org/activity-manager;1'])) {
            ...
            const statusText = autoArchiveUtil.Name + ': ' + (preStatus == autoArchiveService.STATUS_FINISH ? preDetail + ', ' : '') + detail;
            aWindow._autoarchive.timer.initWithCallback(function () { // use timer to make sure I'm the last to setStatusString
              ...
              const func = aWindow.MsgStatusFeedback.setStatusString || aWindow.MsgStatusFeedback.setJSDefaultStatus;
              if (func) func.call(aWindow.MsgStatusFeedback, statusText);
            }, 0, Ci.nsITimer.TYPE_ONE_SHOT);
          }
          [preStatus, preDetail] = [status, detail];
          const menu = doc.getElementById(contextMenuID);
          const label = autoArchive.strBundle.GetStringFromName('mainwindow.menu.' + (status == autoArchiveService.STATUS_RUN ? 'stop' : 'run'));
        ...
        aWindow._autoarchive.prefCallback = function (key) {
          if (key == 'hibernate') {
            const className = 'awsome_auto_archive-hibernate';
            const newValue = autoArchivePref.options[key];
            if (newValue == 0) statusbarIcon.classList.remove(className);
            else statusbarIcon.classList.add(className);
            const scheduleMenu = doc.getElementById(contextMenuScheduleID);
            if (scheduleMenu && scheduleMenu.firstChild && scheduleMenu.firstChild.childNodes) {
              for (const menuitem of scheduleMenu.firstChild.childNodes) {
                if (menuitem.tagName != 'menuitem') continue;
                let value = Number(menuitem.getAttribute('hibernate'));
                if (value > 0) value += Date.now() / 1000;
                value = Math.round(value);
                const checked = ((value <= 0 && newValue == value) || (value > 0 && Math.abs(newValue - value) < 60 * 10)); // Still checked within 10 minutes, this is for TB restart
      ...
      if (autoArchivePref.options.add_context_munu_rule && contextMenuSplit && contextMenuSplit.parentNode) {
        const newMenu = doc.createElementNS(XULNS, 'menuitem');
  ...
  beforePopupShow: function (event) {
    ...
    if (!event.view || !event.view.document || !event.view._autoarchive || !event.view._autoarchive.contextMenuItem) return true;
      ...
      const folderDisplay = event.view.gFolderDisplay;
      if (!folderDisplay || folderDisplay.selectedCount <= 0 || folderDisplay.selectedMessages.length <= 0) shouldShow = false;
        ...
        const tabmail = event.view.document.getElementById('tabmail');
  ...
  onUnLoad: function (event) {
    const aWindow = event.currentTarget;
  ...
  openOption: function (win, msgHdr) {
    ...
    win.openDialog('chrome://awsomeAutoArchive/content/autoArchivePrefDialog.' + xulSuffix, '_blank',
  ...
  addMenuItem: function (menu, doc, parent) {
    const isSubMenu = typeof (menu[2]) === 'object' && menu[2] instanceof Array;
    const item = doc.createElementNS(XULNS, menu[0] == '' ? 'menuseparator' : isSubMenu ? 'menu' : 'menuitem');
    if (menu[0] != '') {
      item.setAttribute('label', menu[0]);
      if (menu[1]) item.setAttribute('image', menu[1]);
      if (isSubMenu) {
        const menupopup = doc.createElementNS(XULNS, 'menupopup');
  ...
  setHibernate: function (event) {
    ...
    if (value > 0) value += Date.now() / 1000;
    const hibernate = Math.round(value);
    autoArchiveLog.info('setHibernate:' + hibernate + ':' + autoArchiveService._status[0]);
    autoArchivePref.setPerf('hibernate', hibernate);
    if (autoArchiveService._status[0] != autoArchiveService.STATUS_RUN) { autoArchiveService.preStart(autoArchivePref.options.startup_delay); }
  },

  createPopup: function (aWindow) {
    const doc = aWindow.document;
    const popupset = doc.createElementNS(XULNS, 'popupset');
    popupset.id = popupsetID;
    const menupopup = doc.createElementNS(XULNS, 'menupopup');
    const menuGroupName = 'awsome_auto_archive-schedule';
  ...
  createRuleBasedOn: function (event) {
    ...
    const msgHdr = folderDisplay.selectedMessages[0];
