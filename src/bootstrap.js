...
function loadIntoWindow (window) {
  ...
  const document = window.document; // XULDocument
  const type = document.documentElement.getAttribute('windowtype'); // documentElement maybe 'messengerWindow' / 'addressbookWindow'
...
var windowListener = {
  onOpenWindow: function (aWindow) {
    const onLoadWindow = function () {
      aWindow.removeEventListener('load', onLoadWindow, false);
      const msgComposeWindow = aWindow.document.getElementById('msgcomposeWindow');
  ...
  observe: function (subject, topic, data) {
    if (topic == 'xul-window-registered') {
      windowListener.onOpenWindow(subject.getInterface(Ci.nsIDOMWindow));
    }
  }
};
